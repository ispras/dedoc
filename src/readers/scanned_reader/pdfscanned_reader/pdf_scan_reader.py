...
from src.extensions import recognized_mimes, recognized_extensions
from src.readers.scanned_reader.data_classes.line_with_location import LineWithLocation
from src.readers.scanned_reader.data_classes.pdf_image_attachment import PdfImageAttachment
from src.readers.scanned_reader.data_classes.tables.scantable import ScanTable
from src.readers.scanned_reader.pdf_base_reader import PdfBase, ParametersForParseDoc
from src.readers.scanned_reader.pdfscanned_reader.ocr.ocr_line_extractor import OCRLineExtractor
from src.readers.scanned_reader.pdfscanned_reader.ocr_utils import set_ocr_thread_limit
from src.readers.scanned_reader.pdfscanned_reader.scan_rotator import ScanRotator
from src.train_dataset.train_dataset_utils import save_page_with_bbox
from src.utils.image_utils import supported_image_types
...
class PdfScanReader(PdfBase):
    ...
    def can_read(self,
        ...
        return ((self.__check_mime(mime, with_archive) or self.__check_path(path, with_archive)) and
                (not document_type or document_type.replace(".", "") in supported_image_types))
    ...
    def __check_path(self, path: str, with_archive: bool) -> bool:
        return (path.endswith(tuple(recognized_extensions.image_like_format)) or
                (path.endswith(tuple(recognized_extensions.archive_like_format)) and with_archive))
