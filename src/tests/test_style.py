...
class TestCodeFormat(unittest.TestCase):
    ...
    def get_files() -> List[str]:
        ...
        manager_config = os.path.abspath(os.path.join(path, "", "manager_config.py"))
        files.append(manager_config)
        config = os.path.abspath(os.path.join(path, "..", "config.py"))
        files.append(config)
        return files
    ...
    def __check_imports(self, file: str, import_not_from_root: list, too_many_blocks: list) -> None:
        ...
        prefix_for_local_imports = ("config", "src")
        if len(blocks) == 3:
