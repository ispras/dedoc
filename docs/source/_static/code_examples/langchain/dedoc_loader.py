...
class DedocBaseLoader(BaseLoader, ABC):
    ...
    Loader enables extracting text, tables and attached files from the given file:
        * `Text` can be split by pages, `dedoc` tree nodes, textual lines (according to the `split` parameter).
        * `Attached files` (when with_attachments=True and need_content_analysis==True) are split according to the `split` parameter.
            For attachments, langchain Document object has an additional metadata field `type`="attachment".
        * `Tables` (when with_tables=True) are not split - each table corresponds to one langchain Document object.
            For tables, Document object has additional metadata fields `type`="table" and `text_as_html` with table HTML representation.
    """
    def __init__(
        self,
        file_path: str,
        ...
        **dedoc_kwargs: Union[str, bool]
        ...
        Args:
            ...
            split: type of document splitting into parts (each part is returned separately), default value "document"
                "document": document text is returned as a single langchain Document object (don't split)
                "page": split document text into pages (works for PDF, DJVU, PPTX, PPT, ODP)
                "node": split document text into tree nodes (title nodes, list item nodes, raw text nodes)
                "line": split document text into lines
            with_tables: add tables to the result - each table is returned as a single langchain Document object
            dedoc_kwargs: parameters used for document parsing via `dedoc` (https://dedoc.readthedocs.io/en/latest/parameters/parameters.html).
                with_attachments: enable attached files extraction, you need also need_content_analysis parameter to get results
                need_content_analysis: enable attached files parsing, works only when with_attachments==True
                recursion_deep_attachments: recursion level for attached files extraction, works only when with_attachments==True
                pdf_with_text_layer: type of handler for parsing PDF documents
                language: language of the document for PDF without a textual layer and images
                pages: page slice to define the reading range for parsing PDF documents
                is_one_column_document: number of columns in case it’s known beforehand for PDF without a textual layer and images
                document_orientation: fix document orientation (90, 180, 270 degrees) for PDF without a textual layer and images
                need_header_footer_analysis: remove headers and footers from the output result for parsing PDF and images
                need_binarization: clean pages background (binarize) for PDF without a textual layer and images
                need_pdf_table_analysis: parse tables for PDF without a textual layer and images
        ...
        self.parsing_parameters = {**dedoc_kwargs, **{"structure_type": "tree" if self.split == "node" else "linear", "document_type": "other"}}
    ...
    def _make_config(self) -> dict:
        """Make configuration for DedocManager according to the file extension and parsing parameters."""
        pass
    ...
    def _parse_subparagraphs(
        ...
        if len(document_tree["subparagraphs"]) > 0:
            for subparagraph in document_tree["subparagraphs"]:
                yield from self._parse_subparagraphs(document_tree=subparagraph, document_metadata=document_metadata)
        else:
            yield Document(page_content=document_tree["text"], metadata={**document_metadata, **document_tree["metadata"]})
    ...
    def _split_document(
        ...
        if split == "document":
            ...
            for node in nodes:
                if node["metadata"]["page_id"] == page_id:
                    ...
                    yield Document(page_content=page_text, metadata={**document_metadata, "page_id": page_id})
            ...
            for node in document_tree["content"]["structure"]["subparagraphs"]:
                ...
                yield Document(page_content=self._json2txt(node), metadata={**document_metadata, **line_metadata})

        elif split == "node":
            yield from self._parse_subparagraphs(document_tree=document_tree["content"]["structure"], document_metadata=document_metadata)
        ...
        for attachment in document_tree["attachments"]:
            yield from self._split_document(document_tree=attachment, split=self.split, additional_metadata={"type": "attachment"})
    ...
    def _get_table(
        ...
        for row in table["cells"]:
            ...
            for cell in row:
                ...
                table_html += f' colspan="{cell["colspan"]}" rowspan="{cell["rowspan"]}">{cell_text}</td>\n'
            table_html += "</tr>\n"
        table_html += "</tbody>\n</table>"
...
class DedocFileLoader(DedocBaseLoader):
    ...
    The file loader automatically detect the file type (with the correct extension).
    ...
    def _make_config(
        ...
        return make_manager_config(file_path=self.file_path, parsing_params=self.parsing_parameters, split=self.split)


class DedocAPIFileLoader(DedocBaseLoader):
    """
    Load files using `dedoc` API.
    The file loader automatically detect the file type (even with the wrong extension).
    ...
    def __init__(
        ...
        **kwargs: Union[str, bool]
        ...
        Args:
            ...
            split: type of document splitting into parts (each part is returned separately), default value "document"
                "document": document is returned as a single langchain Document object (don't split)
                "page": split document into pages (works for PDF, DJVU, PPTX, PPT, ODP)
                "node": split document into tree nodes (title nodes, list item nodes, raw text nodes)
    ...
    def lazy_load(self) -> Iterator[Document]:
        """Lazily load documents."""
        doc_tree = self._send_file(url=self.url, file_path=self.file_path, parameters=self.parsing_parameters)
