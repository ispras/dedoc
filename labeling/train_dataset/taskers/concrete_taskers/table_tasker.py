...
class TableTasker(AbstractTasker):
    ...
    def __init__(self) -> None:
        self.table_path = get_config().get("table_path", "/tmp/tables")
    ...
    def _get_files(self) -> List[File]:
        files = {file.split(".")[0] for file in os.listdir(self.table_path)}
        result = []
        for file_name in sorted(files):
            image_path = os.path.join(self.table_path, f"{file_name}.png")
            json_path = os.path.join(self.table_path, f"{file_name}.json")
