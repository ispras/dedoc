...
class AbstractTasker(ABC):
    ...
    def _add_docker_files(self, archive: zipfile.ZipFile, task_directory: str, dockerfile_directory: str) -> zipfile.ZipFile:
        ...
        docker_path = os.path.join(self.resources_path, dockerfile_directory, "Dockerfile")
        archive.write(filename=docker_path, arcname=f"{task_directory}/Dockerfile")
        readme_path = os.path.join(self.resources_path, dockerfile_directory, "README.md")
        archive.write(filename=readme_path, arcname=f"{task_directory}/README.md")
        run_path = os.path.join(self.resources_path, dockerfile_directory, "run.sh")
