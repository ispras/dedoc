...
class TestPagesCreator(TestCase):
    ...
    def test_pdf(self) -> None:
        ...
        result = self.processor.process([node], PdfBaseConfig())

        self.assertEqual(0, len(result.changed_nodes))
        self.assertEqual(1, len(result.new_nodes))
        self.assertEqual(0, len(result.delete_nodes))

        page_nodes = list(result.new_nodes[node])
    ...
    def test_page_range(self) -> None:
        ...
        for start_page, end_page in [(1, 1), (1, 2), (2, 2), (2, 3), (5, 8), (9, 9)]:
            with self.subTest(start_page=start_page, end_page=end_page):
                config = PdfBaseConfig(start_page=start_page, end_page=end_page)
                result = self.processor.process([node], config)
                self.assertEqual(1, len(result.new_nodes))
                page_nodes = list(result.new_nodes[node])
        ...
        for start_page in (1, 2, 9):
            with self.subTest(start_page=start_page, end_page=None):
    ...
    def test_page_out_of_limit(self) -> None:
        ...
        result = self.processor.process([node], PdfBaseConfig(end_page=11))
        self.assertEqual(1, len(result.new_nodes))
        page_nodes = list(result.new_nodes[node])
