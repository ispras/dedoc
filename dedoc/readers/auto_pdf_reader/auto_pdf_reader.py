...
from dedoc.readers.scanned_reader.pdfscanned_reader.columns_orientation_classifier.columns_orientation_classifier import \
...
from dedoc.readers.auto_pdf_reader.pdf_textlayer_correctness import PdfTextLayerCorrectness
...
class AutoPdfReader(BaseReader):
    ...
    def __init__(self, *, config: dict) -> None:
        ...
        self.scanned_reader = PdfScanReader(config=config)
        self.config = config
    ...
    def _handle_incorrect_text_layer(self,
                                     document_type: str,
        ...
        result = self.scanned_reader.read(path=path, document_type=document_type, parameters=parameters_copy)
        return result
    ...
    def _handle_correct_layer(self,
        ...
        if not is_first_page_correct:
            ...
            prefix = self.scanned_reader.read(path=path, document_type=document_type, parameters=scan_parameters)
