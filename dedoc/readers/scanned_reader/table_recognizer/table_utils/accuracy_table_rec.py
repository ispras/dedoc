...
def save_json(tables: List[ScanTable], number_test_string: str, path_output: str) -> None:
    for i in range(0, len(tables)):
        with open(path_output + "{}_table_{}.json".format(number_test_string, i), "w") as out:
...
def calc_accuracy(path_image: str,
    ...
    for i in range(0, len(image_files)):  # цикл по экземплярам
        name_example = image_files[i].split('.')[0].split('_')[0]
        # predict tables
        image = cv2.imread(path_image + image_files[i], 0)
        clean_images, tables = PdfScanReader(config=get_config()).get_tables([image])
        ...
        for index_table in range(0, len(gt_files)):  # цикл по таблицам на изображении
