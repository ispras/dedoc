...
class ListPatcher:
    ...
    def is_less(self, item1: List[int], item2: List[int]) -> bool:
    ...
    def have_parent(self, items: List[List[int]], parent: List[int]) -> int:
        for item in items[::-1]:
            if self.is_less(parent, item):
                return False
    ...
    def patch_list(self, line: LineWithMeta, items: List[List[int]], levels: List[HierarchyLevel], parent: List[int]):
        if self.have_parent(items, parent):
            while self.is_less(items[-1], parent):
                levels.pop()
                items.pop()

        inserting = []

        while parent != items[-1] and self.is_less(items[-1], parent):
    ...
    def patch(self, lines: List[LineWithMeta]) -> List[LineWithMeta]:
        ...
        for line in lines:
            if not self.is_list(line.line):
                patched_lines.append(line)
                continue

            item = self.get_list_item(line.line)
            parent = self.get_parent(item)

            if items and parent != items[-1]:
                patched_lines.extend(self.patch_list(line, items, levels, parent))
